...
<template>
  ...
  >
    <div
      ...
      :class="[
        tabStyle === 'tab-card' || tabStyle === 'tab-google'
          ? 'border-b border-[var(--velox-card-border)]'
          : ''
      ]"
    >
      <div class="flex-c flex-1 min-w-0 leading-15" style="display: flex">
        ...
        <!-- 快速入口 -->
        <VeloxFastEnter v-if="shouldShowFastEnter && width >= headerBarFastEnterMinWidth">
          <VeloxIconButton icon="ri:function-line" class="ml-3" />
        </VeloxFastEnter>
        ...
        <!-- 顶部菜单 -->
        <VeloxHorizontalMenu v-if="isTopMenu" :list="menuList" />

        <!-- 混合菜单-顶部 -->
        <VeloxMixedMenu v-if="isTopLeftMenu" :list="menuList" />
      </div>
      ...
      <div class="flex-c gap-2.5">
        ...
        >
          <div class="flex-c">
            <VeloxSvgIcon icon="ri:search-line" class="text-sm text-g-500" />
          ...
          <div class="flex-c h-5 px-1.5 text-g-500/80 border border-g-400 rounded">
            <VeloxSvgIcon v-if="isWindows" icon="vaadin:ctrl-a" class="text-sm" />
            <VeloxSvgIcon v-else icon="ri:command-fill" class="text-xs" />
        ...
        >
          <VeloxIconButton icon="ri:translate-2" class="language-btn text-[19px]" />
          <template #dropdown>
            <ElDropdownMenu>
              <div v-for="item in languageOptions" :key="item.value" class="lang-btn-item">
                ...
                >
                  ...
                  <VeloxSvgIcon icon="ri:check-fill" v-if="locale === item.value" />
                </ElDropdownItem>
              </div>
            </ElDropdownMenu>
          </template>
        ...
        <div v-if="shouldShowSettings">
          <ElPopover :visible="showSettingGuide" placement="bottom-start" :width="190" :offset="0">
            <template #reference>
              <div class="flex-cc">
                <VeloxIconButton icon="ri:settings-line" class="setting-btn" @click="openSetting" />
              </div>
            </template>
            <template #default>
              <p
    ...
    <!-- 通知 -->
    <VeloxNotification v-model:value="showNotice" ref="notice" />
  </div>
</template>
...
<script setup lang="ts">
  ...
  const bodyCloseNotice = (e: any): void => {
    ...
    const isNoticePanel = target.closest('.velox-notification-panel')
...
<style lang="scss" scoped>
  ...
  .refresh-btn:hover :deep(.velox-svg-icon) {
    animation: rotate180 0.5s;
  }

  .language-btn:hover :deep(.velox-svg-icon) {
    animation: moveUp 0.4s;
  }

  .setting-btn:hover :deep(.velox-svg-icon) {
    animation: rotate180 0.5s;
  }

  .full-screen-btn:hover :deep(.velox-svg-icon) {
    animation: expand 0.6s forwards;
  }

  .exit-full-screen-btn:hover :deep(.velox-svg-icon) {
    animation: shrink 0.6s forwards;
  }

  .notice-button:hover :deep(.velox-svg-icon) {
    animation: shake 0.5s ease-in-out;
  }

  .chat-button:hover :deep(.velox-svg-icon) {
    animation: shake 0.5s ease-in-out;
  }
