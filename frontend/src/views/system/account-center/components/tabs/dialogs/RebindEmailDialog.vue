<script setup lang="ts">
  ...
  const currentEmailSendTip = computed(() =>
    t('pages.system.accountCenter.security.email.dialog.sendToCurrentEmail', {
      email: props.currentEmail || ''
    })
  )
  const dialogTitle = computed(() =>
    props.currentEmail?.trim()
      ? t('pages.system.accountCenter.security.email.dialog.title')
      : t('pages.system.accountCenter.security.email.dialog.bindTitle')
  )
    ...
    currentEmailCode: [
      {
        ...
        message: t('pages.system.accountCenter.security.email.dialog.currentEmailCodeRequired'),
        trigger: 'blur'
      }
    ],
    totpCode: [
      {
        ...
        message: t('pages.system.accountCenter.security.email.dialog.totpCodeRequired'),
        trigger: 'blur'
      }
    ],
    currentPassword: [
      {
        ...
        message: t('pages.system.accountCenter.security.email.dialog.currentPasswordRequired'),
        trigger: 'blur'
      }
    ]
  }
  ...
  const bindRules: FormRules = {
    newEmail: [
      {
        required: true,
        message: t('pages.system.accountCenter.security.email.dialog.emailRequired'),
        trigger: 'blur'
      },
      {
        type: 'email',
        message: t('pages.system.accountCenter.security.email.dialog.emailInvalid'),
        trigger: 'blur'
      }
    ],
    newEmailCode: [
      {
        required: true,
        message: t('pages.system.accountCenter.security.email.dialog.codeRequired'),
        trigger: 'blur'
      }
    ]
  }
  ...
  const handleProofExpired = (error: unknown) => {
    ...
    ElMessage.warning(t('pages.system.accountCenter.security.email.dialog.proofExpired'))
    step.value = 'verify_identity'
    proofTicket.value = ''
    proofExpiresInSeconds.value = 0
    bindForm.newEmailCode = ''
  ...
  const sendProofCode = async () => {
    ...
    try {
      ...
      ElMessage.success(t('pages.system.accountCenter.security.email.dialog.codeSent'))
      startCooldown('proof')
    } finally {
  ...
  const verifyIdentity = async () => {
    ...
    try {
      ...
      ElMessage.success(t('pages.system.accountCenter.security.email.dialog.verifySuccess'))
    } finally {
  ...
  const sendBindCode = async () => {
    ...
    if (bindForm.newEmail.trim() === (props.currentEmail || '').trim()) {
      ElMessage.warning(t('pages.system.accountCenter.security.email.dialog.emailSame'))
      return
    }
  ...
  const submit = async () => {
    ...
    try {
      ...
      ElMessage.success(
        props.currentEmail?.trim()
          ? t('pages.system.accountCenter.security.email.dialog.success')
          : t('pages.system.accountCenter.security.email.dialog.bindSuccess')
      )
  ...
  >
    <div class="mb-4 text-xs text-g-500">
      {{
        step === 'verify_identity'
          ? t('pages.system.accountCenter.security.email.dialog.verifyStep')
          : t('pages.system.accountCenter.security.email.dialog.bindStep')
      }}
    </div>
    ...
    >
      <div class="mb-4 text-sm text-g-600">
        {{ t('pages.system.accountCenter.security.email.dialog.proofTip') }}
      </div>

      <ElFormItem v-if="isCurrentEmailProof" :label="currentEmailSendTip" prop="currentEmailCode">
        <div class="flex gap-2 w-full">
          <ElInput
            v-model="identityForm.currentEmailCode"
            :placeholder="
              t('pages.system.accountCenter.security.email.dialog.currentEmailCodePlaceholder')
            "
            maxlength="6"
          />
          <ElButton
          ...
          >
            {{
              proofCooldown > 0
                ? `${proofCooldown}s`
                : t('pages.system.accountCenter.security.email.dialog.verifyCurrentEmailCode')
            }}
          </ElButton>
        </div>
      </ElFormItem>
      ...
      <ElFormItem
        ...
        :label="t('pages.system.accountCenter.security.email.dialog.totpCode')"
        prop="totpCode"
      >
        <ElInput
          v-model="identityForm.totpCode"
          :placeholder="t('pages.system.accountCenter.security.email.dialog.totpCodePlaceholder')"
          maxlength="6"
        />
      </ElFormItem>
      ...
      <ElFormItem
        v-else
        :label="t('pages.system.accountCenter.security.email.dialog.currentPassword')"
        prop="currentPassword"
      >
        <ElInput
          v-model="identityForm.currentPassword"
          :placeholder="
            t('pages.system.accountCenter.security.email.dialog.currentPasswordPlaceholder')
          "
          show-password
          type="password"
        />
    ...
    <ElForm v-else ref="bindFormRef" :model="bindForm" :rules="bindRules" label-position="top">
      <ElFormItem
        :label="t('pages.system.accountCenter.security.email.dialog.newEmail')"
        prop="newEmail"
      >
        <ElInput
          v-model="bindForm.newEmail"
          :placeholder="t('pages.system.accountCenter.security.email.dialog.newEmailPlaceholder')"
        />
      </ElFormItem>

      <ElFormItem
        :label="t('pages.system.accountCenter.security.email.dialog.code')"
        prop="newEmailCode"
      >
        <div class="flex gap-2 w-full">
          <ElInput
            v-model="bindForm.newEmailCode"
            :placeholder="t('pages.system.accountCenter.security.email.dialog.codePlaceholder')"
            maxlength="6"
          />
          <ElButton
          ...
          >
            {{
              bindCooldown > 0
                ? `${bindCooldown}s`
                : t('pages.system.accountCenter.security.email.dialog.sendCode')
            }}
          </ElButton>
        </div>
      </ElFormItem>
    ...
    <template #footer>
      ...
      >
        {{ t('pages.system.accountCenter.security.email.dialog.next') }}
      </ElButton>
